...
export default function SearchBarWithSuggestion() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', width: 255, height: 'auto', backgroundColor: 'lightgrey' }}>
        <div style={{ backgroundColor: 'black', color: 'white' }}> {txtValue.length != 0 ? <>Suggestions (up to 5)</> : <>Previous Searches (up to 5)</>}</div>
        {txtValue.length != 0 ? <>{ShowData()}</> : <>{ShowRecent()}</>}
      </div>
      <div style={{ marginTop: 100, color: 'darkgreen', fontSize: 30, fontWeight: 'bold' }}>{text}</div>
    </div>
  )
}
